    ...
    <div class="v-item__header">
            ...
            {foreach $core.actions as $name => $action}
                <li>
                    {if 'action-favorites' == $name}
                        {printFavorites item=$item}
                    {else}
                        <a data-toggle="tooltip" title="{$action.title}" {foreach $action.attributes as $key => $value}{$key}="{$value|escape}" {/foreach}>
        ...
        <h2>
            {if $item.logo}
                ...
                {ia_image type='thumbnail' file=$logo title=$item.title class='img-circle' width=30}
            {/if}
...
<div class="row">
        ...
        <table class="v-item-table">
            <tbody>
                ...
                <tr>
                    ...
                    <td>{$item.company_address|escape}</td>
                </tr>
                <tr>
                    <td>{lang key='field_autos_service_company_phone'}</td>
                    <td>{$item.company_phone|escape}</td>
                </tr>
                ...
                {if $item.company_skype}
                    <tr>
                        ...
                        <td><a href="call:{$item.company_skype|escape}">{$item.company_skype|escape}</a></td>
                    </tr>
                {/if}
            </tbody>
        </table>
    ...
    <div class="col-md-8">
            ...
            <div class="ia-item-view__gallery">
                <div class="fotorama"
                    ...
                    {foreach $item.pictures as $entry}
                        <a class="ia-item-view__gallery__item" href="{ia_image file=$entry url=true type='large'}">{ia_image type='large' file=$entry}</a>
                    {/foreach}
                </div>
            </div>
        {/if}
        ...
        <div class="m-t-lg">
            ...
            {include 'item-view-tabs.tpl' isView=true exceptions=['logo', 'pictures', 'title', 'categories', 'description', 'company_address', 'company_phone', 'company_website', 'company_skype'] class='ia-item-view-tabs'}
