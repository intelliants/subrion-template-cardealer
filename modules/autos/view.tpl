<div class="v-item">
    <div class="row">
        ...
        <div class="col-md-6">
            <div class="v-item__header">
                ...
                <h2><span>{$item.model|escape}, {$item.release_year}</span></h2>
                ...
                <div class="v-item-header__info">
                    ...
                    <span class="v-item__header__info__item">{lang key='field_auto_condition'}: <a href="{$searchPageUrl}condition:{$item.condition}/"><b>{lang key="field_autos_condition+{$item.condition}"}</b></a></span>
                ...
                <div class="ia-car-info ia-car-info--block">
                    <span class="ia-car-info__icon ia-car-info__icon--body">{lang key='field_auto_body_type'}: <a href="{$smarty.const.IA_URL}search/cars/body:{$item.body_type}/"><b>{lang key="field_autos_body_type+{$item.body_type}"}</b></a></span>
                    {if $item.engine}
                        <span class="ia-car-info__icon ia-car-info__icon--engine">{lang key='field_auto_engine'}: <b>{lang key="field_autos_engine+{$item.engine}"}{if $item.engine_type} {lang key="field_autos_engine_type+{$item.engine_type}"}{/if}{if $item.engine_size} {$item.engine_size}{/if}</b></span>
                    {/if}
                    {if $item.transmission}
                        <span class="ia-car-info__icon ia-car-info__icon--transmission">{lang key='field_auto_transmission'}: <a href="{$smarty.const.IA_URL}search/cars/transmission:{$item.transmission}/"><b>{lang key="field_autos_transmission+{$item.transmission}"}</b></a></span>
                    {/if}
                ...
                <div class="v-item__header__social">
                    ...
                    <a href="mailto:?subject={$item.model|escape},{$item.release_year}&body={$item.model}, {$item.release_year}:{$smarty.const.IA_SELF|escape:'url'}" target="_blank" title="Email" class="btn btn-sm btn-email"><i class="fa fa-envelope"></i></a>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="v-item-info">
        ...
        {include 'item-view-tabs.tpl' isView=true exceptions=array('model', 'price', 'release_year', 'condition', 'mileage', 'engine', 'engine_type', 'engine_size', 'horse_power', 'transmission', 'pictures', 'fuel_type', 'body_type', 'exterior_color', 'door_count', 'metallic', 'interior_color', 'interior_leather', 'drive_type', 'vin_code', 'additional_info', 'options_features') class='v-item-info__section'}
